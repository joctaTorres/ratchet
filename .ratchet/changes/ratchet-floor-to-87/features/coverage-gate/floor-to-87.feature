Feature: Coverage-gate floor raised to 87
  As a maintainer ratcheting test coverage upward
  I want the enforced default coverage floor lifted from 80 to 87
  So that the now-covered core utilities and helpers lock in their gain and the floor can only climb toward 95%, never silently regress
  ...
  # environment variable. This phase's two upstream test changes
  # (core-util-tests, utils-helper-tests) plus the coverage-scope change lifted
  # measured total line coverage to ~87%. This change raises the enforced default
  # floor from 80 to the phase target (87) and proves the raised floor's
  # green-at/above, red-below behavior at the unit level. The measured coverage
  # that satisfies the raised floor is delivered by the upstream changes this
  # change is sequenced after.

  Scenario: The default enforced floor is raised to 87
    ...
    Then the resolved threshold is 87
    And the threshold is strictly greater than the previous 80 floor

  Scenario: The gate is green when coverage is at the raised floor
    Given a coverage summary reporting a total line coverage of 87%
  ...
  Scenario: The gate is green when coverage is above the raised floor
    Given a coverage summary reporting a total line coverage of 88%
    And no COVERAGE_THRESHOLD override is set
    When the coverage gate runs
    Then the gate signal is green
    And the gate exits 0
  ...
  Scenario: The gate is red when coverage is below the raised floor
    Given a coverage summary reporting a total line coverage of 86.5%
    And no COVERAGE_THRESHOLD override is set
    When the coverage gate runs
    Then the gate signal is red
    And the gate exits 1
    And a reason names the measured coverage and the required threshold of 87
  ...
  Scenario: A non-numeric COVERAGE_THRESHOLD falls back to the raised default
    ...
    When the enforced threshold is resolved from the environment
    Then the resolved threshold is the raised default of 87
