...
- [ ] 3.1 In `rex-sidecar-runtime.ts`, add an `image` option and derive `REX_MOUNT_HOST`/`REX_MOUNT_CONTAINER` + the docker `REX_WORKDIR` from the project root; thread them to the bootstrap options.
- [ ] 3.2 In `rex-bootstrap.ts`, pass `REX_IMAGE`/`REX_MOUNT_HOST`/`REX_MOUNT_CONTAINER` through to the sidecar env (mirroring the existing `REX_LOCUS`/`REX_WORKDIR` passthrough).
- [ ] 3.3 In `engine.ts` `selectRuntime`, pass the resolved `image` to `makeRexSidecarRuntime` when `locus === 'docker'`; assert no other engine/renderer code path branches on locus.
...
- [ ] 4.1 Add a Docker daemon pre-flight in the docker bootstrap branch (probe `docker info`/`version` via the injected `run` seam, short timeout); on failure throw `RexBootstrapError` with an actionable install/start-Docker message naming `locus=docker`.
...
- [ ] 5.1 In `rex-bootstrap.ts`, install `swe-rex[docker]==1.4.0` (or add `aiohttp`) when the docker locus is requested; extend the readiness marker to record the installed extra so a local-only cached venv is rebuilt on first docker use.
