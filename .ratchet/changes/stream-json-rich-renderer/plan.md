...
- [ ] 5.2 Create the phase proof-of-work `test/e2e/render-stream-json.sh`: ALWAYS replay the canned fixtures through the renderer (no external deps) and capture the rendered transcript to a file for the judge; add a live-claude path that SKIPs (explicit SKIP line, exit 0) when `claude`/Python/built dist is absent. Pattern after `test/e2e/rex-local-stream.sh`.
- [ ] 5.3 Run `pnpm build` then `bash test/e2e/render-stream-json.sh`; confirm the captured artifact streams assistant text, tool calls, tool results, and a final summary, and that the malformed line degraded to raw without crashing.
