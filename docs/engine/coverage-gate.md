...
|---|---|---|
| `COVERAGE_THRESHOLD` | `72` | The enforced minimum line-coverage percentage required for `green`. Overrides the built-in default when set to a finite number; a missing, empty, or non-numeric value falls back to the default of `72`. |
...
The default of `72` is a ratchet point: it is raised as coverage is added and
