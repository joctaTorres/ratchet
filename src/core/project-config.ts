 ...
 * - Helps understand Ratchet's data interfaces at a glance
 * - Single source of truth for type and validation
 * - Consistent with other Ratchet schemas
 */
...
/**
 * Read and parse .ratchet/config.yaml from project root.
...
export function readProjectConfig(projectRoot: string): ProjectConfig | null {
  ...
  if (!existsSync(configPath)) {
    configPath = path.join(projectRoot, RATCHET_DIR_NAME, 'config.yml');
    if (!existsSync(configPath)) {
  ...
  try {
    ...
    if (!raw || typeof raw !== 'object') {
      console.warn(`.ratchet/config.yaml is not a valid YAML object`);
      return null;
    }
...
export function suggestSchemas(
  ...
  let message = `Schema '${invalidSchemaName}' not found in .ratchet/config.yaml\n\n`;
