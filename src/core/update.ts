 ...
 * Update Command
 *
 * Refreshes Ratchet skills and commands for configured tools.
...
import { AI_TOOLS, RATCHET_DIR_NAME } from './config.js';
import {
  generateCommands,
...
export class UpdateCommand {
  ...
  async execute(projectPath: string): Promise<void> {
    ...
    const ratchetPath = path.join(resolvedProjectPath, RATCHET_DIR_NAME);

    // 1. Check ratchet directory exists
    if (!await FileSystemUtils.directoryExists(ratchetPath)) {
      throw new Error(`No Ratchet directory found. Run 'ratchet init' first.`);
    }

    // 2. Perform one-time migration if needed before any legacy upgrade generation.
    // Use detected tool directories to preserve existing rct skills/commands.
    ...
    if (configuredTools.length === 0 && newlyConfiguredTools.length === 0) {
      ...
      console.log(chalk.dim('Run "ratchet init" to set up tools.'));
      return;
    }
    ...
    const toolStatuses = configuredTools.map((toolId) => {
      const status = getToolVersionStatus(resolvedProjectPath, toolId, RATCHET_VERSION);
    ...
    if (updatedTools.length > 0) {
      console.log(chalk.green(`✓ Updated: ${updatedTools.join(', ')} (v${RATCHET_VERSION})`));
    }
  ...
  private displayUpToDateMessage(toolStatuses: ToolVersionStatus[]): void {
    ...
    console.log(chalk.green(`✓ All ${toolStatuses.length} tool(s) up to date (v${RATCHET_VERSION})`));
  ...
  private displayUpdatePlan(
    ...
    const updates = toolsToUpdate.map((toolId) => {
      ...
      if (status?.needsUpdate) {
        ...
        return `${status.toolId} (${fromVersion} → ${RATCHET_VERSION})`;
      }
  ...
  private detectNewTools(projectPath: string, configuredTools: string[]): void {
    ...
    if (newTools.length > 0) {
      ...
      console.log(
        chalk.yellow(
          `Detected new ${toolNoun}: ${newToolNames.join(', ')}. Run 'ratchet init' to add ${pronoun}.`
        )
      );
    }
  }
  ...
  private displayExtraWorkflowsNote(
    ...
    if (extraWorkflows.length > 0) {
      console.log(chalk.dim(`Note: ${extraWorkflows.length} extra workflows not in profile (use \`ratchet config profile\` to manage)`));
    }
  }
  ...
  private displayOldCoreCustomProfileNote(profile: Profile, workflows?: readonly string[]): void {
    ...
    console.log(chalk.dim('Run `ratchet config profile core` and then `ratchet update` to add sync.'));
  }
  ...
  /**
   * Detect and handle legacy Ratchet artifacts.
