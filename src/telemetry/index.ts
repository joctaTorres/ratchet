 ...
 * - Opt-out via RATCHET_TELEMETRY=0 or DO_NOT_TRACK=1
...
// PostHog API key - intentionally blank for ratchet (telemetry neutralized).
// When empty, getClient() short-circuits to null and no events are ever sent.
 ...
 * - No PostHog API key is configured (default for ratchet)
 */
export function isTelemetryEnabled(): boolean {
  // Telemetry is neutralized: with no API key configured, never enable.
  if (!POSTHOG_API_KEY) {
    return false;
  }

  // Check explicit opt-out
  if (process.env.RATCHET_TELEMETRY === '0') {
    return false;
  }
...
function getClient(): PostHog | null {
  // Telemetry neutralized: without an API key there is no client.
  if (!POSTHOG_API_KEY) {
    return null;
  }
  if (!posthogClient) {
 ...
 * @param version - The ratchet version
 */
...
export async function maybeShowTelemetryNotice(): Promise<void> {
  ...
  try {
    ...
    // Display notice
    console.log(
      'Note: ratchet collects anonymous usage stats. Opt out: RATCHET_TELEMETRY=0'
    );
