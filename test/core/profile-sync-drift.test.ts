...
describe('profile sync drift detection', () => {
  ...
  beforeEach(() => {
    tempDir = path.join(os.tmpdir(), `ratchet-profile-sync-drift-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    fs.mkdirSync(path.join(tempDir, '.ratchet'), { recursive: true });
  });
